...
//   // Fetch eligible OS
//   useEffect(() => {
//     if (activeTab === 'elaboracao') {
//       setLoadingEligible(true);
//       api.get('/api/faturamento/os-elegiveis')
//         .then(r => setEligibleOS(r.data?.data || []))
...
//     api.get(`/api/faturamento/os-total?ids=${selectedOS.join(',')}`)
//       .then(r => { setValorTotal(r.data?.data?.total || 0); setDetalhesOS(r.data?.data?.detalhes || []); })
//       .catch(e => console.error(e));
//   }, [selectedOS]);
...
//     setSaving(true);
//     try {
//       await api.post('/api/faturamento', {
//         os_ids: selectedOS, valor_servico: valorTotal,
//         data_vencimento: dataVencimento, valor_desconto: desconto ? Number(desconto.replace(',', '.')) : 0,
...
//     } catch (e: any) {
//       alert(e.response?.data?.message || 'Erro ao salvar fatura.');
...
//     try {
//       await api.put(`/api/faturamento/${id}/pagar`, { valor_pagamento: Number(payValue.replace(',', '.')) });
//       setFaturas(prev => prev.map(f => f.id_faturamento === id ? { ...f, status_pagamento: 'Pago', data_pagamento: new Date().toISOString() } : f));
//       setPayingId(null); setPayValue('');
//     } catch (e: any) { alert(e.response?.data?.message || 'Erro ao registrar pagamento.'); }
//   };

//   const handleCancelar = async (id: number) => {
//     if (!confirm('Deseja cancelar esta fatura?')) return;
//     try {
//       await api.delete(`/api/faturamento/${id}`);
...
//     api.get(`/api/faturamento/relatorio?mes=${mesSel}&ano=${anoSel}`)
//       .then(r => setRelatorio(r.data?.data || []))
//       .catch(e => console.error(e))
//       .finally(() => setLoadingRelatorio(false));
//   };

//   useEffect(() => { if (activeTab === 'relatorio') fetchRelatorio(); }, [activeTab, mesSel, anoSel]);
...
//                       <span className="text-gray-500">Serviços: {fmt(d.total_servicos)} | Peças: {fmt(d.total_pecas)} | Rebob: {fmt(d.total_rebobinamentos)}</span>
//                       <span className="font-bold text-gray-800">{fmt(d.total)}</span>
//                     </div>
//                   ))}
